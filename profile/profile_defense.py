...
kv_file_path = os.path.join(os.path.dirname(__file__), 'profile_defense.kv')
...
class ProfileDefenseScreen(Screen):
    ...
    def on_pre_enter(self):
        ...
        self.ids.left_defense_checkbox.active = user_profile.get('left_defense', False)
        self.ids.right_defense_checkbox.active = user_profile.get('right_defense', False)
    ...
    def on_checkbox_active(self, checkbox, value, position):
        self.save_defense(position, value)

    def save_defense(self, position, value):
        user_profile = load_user_profile()
        user_profile[position] = value
        user_profile['defense'] = any([
            user_profile.get('left_defense', False),
            user_profile.get('right_defense', False)
        ])
        ...
        print(f"Defense - {position}: {user_profile[position]}")
        print(f"Defense: {user_profile['defense']}")
